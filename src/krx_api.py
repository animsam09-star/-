...
테마지수 구성종목이 없다.** 그게 원래 수평축 그룹(groups.py)의 원천이었다.
인증키만 쓰는 구성에서는 대신 산업 노드(`I:*`)의 소속 종목을 그룹으로 쓴다
(groups.fetch_industry_groups). pykrx 자격 증명이 있으면 기존 경로도 함께 돈다.

성격이 다르다는 점은 알고 써야 한다. ETF PDF는 '운용사가 같은 바구니에 담았다'는
시장의 판단이고, 산업 노드는 '사업보고서가 같은 산업이라고 적었다'는 사업 실체다.
